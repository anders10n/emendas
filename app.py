...
# Inicializa o APP com suporte embutido a CORS global
app = Flask(__name__)
CORS(app)

@app.route('/')
def index():
    return "API Extrator de Emendas: ONLINE E OPERANTE!", 200

@app.route('/health')
def health():
    return jsonify({"status": "ok", "message": "API Extrator de Emendas está rodando saudavelmente"}), 200
